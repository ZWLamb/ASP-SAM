...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    ...
    if net == 'sam':
        ...
        if args.encoder not in options:#tiny_vit
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
            net = sam_model_registry[args.encoder](args,checkpoint=args.sam_ckpt) #build_sam_vit_t(args,checkpoint="./mobile_sam.pt")
