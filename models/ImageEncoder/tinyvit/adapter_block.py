...
from .Multiscale_Linear_Attention import LiteMLA
...
class TinyViTAdapterBlock(nn.Module):
    def __init__(self, args, dim, input_resolution, num_heads, window_size=7,
        ...
        self.msa = LiteMLA(in_channels=self.dim, out_channels=self.dim, scales=(3,5))

    def _forward(self, x):
        H, W = self.input_resolution #32*32
        B, L, C = x.shape #2 1024 128
        assert L == H * W, "input feature has wrong size"
        res_x = x
        if H == self.window_size and W == self.window_size:
            x = self.attn(x)
        else:
            x = x.view(B, H, W, C) # 2 128 128 128
