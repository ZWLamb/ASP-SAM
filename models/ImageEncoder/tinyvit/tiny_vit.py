...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x) # 8*64*64*64
        x = self.act(x)

        x = self.conv2(x)
        x = self.act(x)
        x = self.conv3(x) #8*128*32*32
...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        for blk in self.blocks:#MBConv * depth
...
class TinyViT(nn.Module):
    def __init__(self, args, img_size=224, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = PatchEmbed(in_chans=in_chans, #3
                                      ...
                                      resolution=img_size,  #1024
                                      activation=activation) # Conv Ac Conv

        patches_resolution = self.patch_embed.patches_resolution #256，256
        self.patches_resolution = patches_resolution #（256，256）

        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate,
                                                sum(depths))]  # stochastic depth decay rule [全0]

        # build layers
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers): #num_layers = 4  [64,128,160,320]
            kwargs = dict(dim=embed_dims[i_layer], #[64,128,160,320]
                        input_resolution=(patches_resolution[0] // (2 ** (i_layer-1 if i_layer == 3 else i_layer)),
                                patches_resolution[1] // (2 ** (i_layer-1 if i_layer == 3 else i_layer))),# 256 128 64 64
                        #   input_resolution=(patches_resolution[0] // (2 ** i_layer),
                        #                     patches_resolution[1] // (2 ** i_layer)),
                          depth=depths[i_layer],#2 2 6 2
                          drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],#drop = 0
                          downsample=PatchMerging if (
                              i_layer < self.num_layers - 1) else None,#除了最后一个layer为None，其余Patcherging
            ...
            if i_layer == 0:
                layer = ConvLayer(
                    conv_expand_ratio=mbconv_expand_ratio,#4
                    **kwargs,
                )
            else:
                layer = BasicLayer(
        ...
        self.head = nn.Linear(
            embed_dims[-1], num_classes) if num_classes > 0 else torch.nn.Identity()  #Linear(320,1000)
    ...
    def forward_features(self, x):
        # x: (N, C, H, W) #2*3*256*256
        x = self.patch_embed(x) #2*3*256*256->2*64*64*64

        x = self.layers[0](x) #ConvLayer 8*1024*128   (1024=32*32)
        start_i = 1

        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x = layer(x) #BasicLayer #8*256*320  (256=16*16)
        B,_,C=x.size()
