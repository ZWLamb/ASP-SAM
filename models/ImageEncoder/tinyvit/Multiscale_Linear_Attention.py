...
class ConvLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size=3, stride=1, dilation=1, groups=1,
                 use_bias=False, dropout=0, norm="bn2d", act_func="gelu", ):
...
if __name__ == '__main__':
    block = LiteMLA(in_channels=128, out_channels=128, scales=(3,5))  # scales: 单尺度:(5,); 多尺度:(3,5)
    input1 = torch.rand(3, 128, 32, 32)
    output = block(input1)
    print(output.size())
